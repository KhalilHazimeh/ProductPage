...
if(isset($_GET['showEditModal']) && isset($_GET['id'])){
    $id = $_GET['id'];
    $query = "SELECT * FROM option_values WHERE id = '" . $_GET['id'] . "'";
...
$query = "SELECT option_id, id, value_name FROM option_values ORDER BY option_id";
$result = $conn->query($query);
...
<div class="row">
                ...
                <tbody>
                    <?php
					 foreach ($result as $row) {
                        ...
                        $queryOptionName = "SELECT name FROM options WHERE id = $optionId";
                        $resultOptionName = $conn->query($queryOptionName);

                        if ($resultOptionName->num_rows > 0) {
                            $rowOptionName = $resultOptionName->fetch_assoc();
                            $optionName = $rowOptionName['name'];
                        } else {
                            $optionName = 'Unknown';
                        }
                        ...
                        echo '<td>';
                        echo '<a href="options.php?showEditModal=1&id='.$row['id'].'" data-id="' . $row['id'] . '" class="edit"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>';
                        echo '<a href="options.php?showDeleteModal=1&id='.$row['id'].'" data-id="' . $row['id'] . '" class="delete" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>';
                        echo '</td>';
                        echo '</tr>';
                    }
                    ?>
		...
		<div class="modal-dialog">
			<div class="modal-content">
                <form action="edit_option.php?edit_option_id=<?php echo $editedOption['id']; ?>" method="POST">
					...
					<div class="modal-body">
                        ...
                        <select name="title" class="form-control" required>
                            ...
                            while ($row = $result->fetch_assoc()) {
                                $selected = ($row['id'] == $editedOption['option_id']) ? "selected" : "";
                        ...
                        <div class="form-group">
                            <label>Value</label>
                            <input name="value" type="text" class="form-control" required value="<?php echo $editedOption['value_name']; ?>">
                        </div>  
                    </div>
            ...
            <form method="post" action="add_option.php">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input name="value_name" type="text" class="form-control" required placeholder="Enter option value">
                    </div>
